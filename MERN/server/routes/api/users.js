...
module.exports = (apiRoutes) => {
    ...
    apiRoutes.get(`/${SERVICE_CONST.GET_USER_LIST}/:id`, async (req, res) => {
        if (req.params.id !== 'null') {
            let decryptedString = cryptr.decrypt(req.params.id);
            await Users.find({'_id': {$ne: decryptedString}}, async (error, users) => {
                if (users.length > 0) {

                    var contr = new UserController();
                    await UsersDetails.find({'userId': {$ne: decryptedString}}, async (error, details) => {
                        let list = await contr.getuserList(users);
            ...
            Users.find({'_id': decryptedString}, (error, users) => {
                if (users.length > 0) {
                    ...
                    UsersDetails.find({'userId': users[0]._id}, (error, details) => {
                        let list = contr.getuserList2(users);
                        let detail = contr.getUserDetails(details);
                        list.forEach((val, i) => {
                            let id = val._id;
                            detail.forEach((dval, k) => {
    ...
    apiRoutes.get(`/${SERVICE_CONST.ACCEPT_FRIEND_LIST}/:id`, async (req, res) => {
        var contr = new UserController();
        if (req.params.id !== 'null') {
            ...
            await Users.aggregate([
                ...
                ;
                if (results.length > 0) {
                    res.json({status: "success", list: await contr.getuserList(results[0].finaldata)});
                } else {
                    res.json({status: "success", message: "No record found!!!!"});
                }
            });
    ...
    // status update
    apiRoutes.post(`/${SERVICE_CONST.STATUS_UPDATE}`, function (req, res) {
     
        

        Status.deleteOne({postby: mongoose.Types.ObjectId(cryptr.decrypt(req.body.postby))}, (data) => {
        ...
        status.save((err, data) => {
            ...
            let messgae = "Status updated successfully";
            res.json({status: "success", message: messgae, commentdata: data});
        });
    ...
    apiRoutes.post(`/${SERVICE_CONST.GET_STATUS}`, function (req, res) {
     
        
        let decryptedString = cryptr.decrypt(req.body.postby);
            Status.find({'postby': decryptedString}, (error, status) => {
                if (status.length > 0) {
                    //console.log(status[0]['status']);
                    res.json({status: "success", user_status: status[0]['status']});
                    
                } else {

                    res.json({status: "success", message: "I'm using react app!"});
                }
            });
